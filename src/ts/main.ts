...
document.getElementById("create-instrument-button").addEventListener("click", () => {
	...
	document
		...
		.insertAdjacentHTML(
			"beforeend",
			`<div class="instrument" id="${instrumentData.id}"><form class="instrument__update"><h2>Update instrument ${instrumentData.id}</h2><div class="create__notes input-container"><p class="instrument_description">Write your notes here</p><textarea name="notes"></textarea></div><div class="create__bpm input-container"><p class="instrument_description">Write song bmp</p><input type="number" name="bpm"value="${instrumentData.bpm}"min="0"max="1000"step="5"/></div><div class="create__type input-container"><p class="instrument_description">Select wave</p><select name="type"><option value="sine">Sinusoidal</option><option value="sawtooth">Sawtooth</option><option value="square">Square</option><option value="triangle">Triangle</option></select></div><button type="submit">Update</button><input type="hidden" name="id" value="${instrumentData.id}"/></form><form class="instrument__adsr"><h2>Configure ADSR</h2><div class="adsr__a input-container"><p class="instrument_description">A</p><input type="number" name="a" value="50" min="0" max="1000" step="10"/></div><div class="adsr__d input-container"><p class="instrument_description">D</p><input type="number" name="d" value="50" min="0" max="5000" step="50"/></div><div class="adsr__s input-container"><p class="instrument_description">S</p><input type="number" name="s" value="1" min="0" max="1" step="0.1"/></div><div class="adsr__r input-container"><p class="instrument_description">R</p><input type="number" name="r" value="400" min="0" max="5000" step="100"/></div><button type="submit">Set</button><input type="hidden" name="id" value="${instrumentData.id}"/></form><div class="instrument-control-container form"><h2>Control instrument</h2><div class="instrument-control-forms-container"><form class="instrument__control"><input type="hidden" name="id" value="${instrumentData.id}"/><button type="submit">Start</button><input type="hidden" name="control" value="start"/></form><form class="instrument__control"><input type="hidden" name="id" value="${instrumentData.id}"/><button type="submit">Stop</button><input type="hidden" name="control" value="stop"/></form><form class="instrument__control"><input type="hidden" name="id" value="${instrumentData.id}"/><button type="submit">Pause/Resume</button><input type="hidden" name="control" value="interrupt"/></form></div></div></div>`
		);
